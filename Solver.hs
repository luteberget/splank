...
plan :: (String -> IO ()) -> Int -> Problem -> IO (Maybe Plan)
plan logmsg maxN problem = withNewSolver $ \s -> do
  ...
  where
    ...
    solve s n state plan = do
      ...
      if b then do
        ...
        if n < maxN then do 
          logmsg $ "--> Adding action " ++ (show n)
          next <- nextState s problem state
          case next of
            Nothing -> do logmsg "=> No further actions possible"
                          return Nothing
            Just (actions, state) -> do
               logmsg $ "    (" ++ (show (Map.size actions)) ++ " choices)"
               solve s (n+1) state (plan ++ [actions])
        else do 
          logmsg "=> Maximum number of actions reached"
          return Nothing
